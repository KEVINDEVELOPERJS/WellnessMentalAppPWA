...
def create_final_web_document_with_correct_images():
    """Crea el documento final de la app web usando las imágenes correctas del documento FOTOS DE MODULOS APP WEB.docx"""
    ...
    # Obtener lista de imágenes extraídas (limpiar carpeta primero)
    if os.path.exists(images_folder):
        # Eliminar imágenes anteriores si es necesario
        existing_files = glob.glob(os.path.join(images_folder, 'image*.png'))
    ...
    # Obtener lista de imágenes extraídas del documento web
    extracted_images = []
    if os.path.exists(images_folder):
    ...
    # Mapeo de secciones a imágenes (basado en las 44 imágenes del documento web)
    # Este mapeo debe ajustarse según el orden real de las imágenes en el documento
    image_mapping = {
        ...
        'Alertas': [32, 33, 34],      # 3 imágenes para alertas
        'Misiones': [35, 36],         # 2 imágenes para misiones
        'Videos': [37, 38],           # 2 imágenes para videos
        'Pausas': [39, 40],           # 2 imágenes para pausas
        'Extra': [41, 42, 43]         # 3 imágenes extra si las hay
    }
    ...
    add_section_with_content(
        ...
        image_path=get_image('Alertas', 2)
    )
    ...
    add_section_with_content(
        ...
        image_path=get_image('Misiones', 0)
    )
    ...
    add_section_with_content(
        ...
        image_path=get_image('Misiones', 1)
    )
    ...
    output_path = r'C:\Users\Admin\Documents\INGENIERIA DE SOFTWARE\IV SEMESTRE\ARCHIVOS (.pptx .docx .pdf .xlsx)\APPS WELLNESS MENTAL\WellnessMentalApp(WEB)\INFORME_FINAL_INTEGRADO_WEB_IMAGENES_CORRECTAS_V2.docx'
    doc.save(output_path)
    print(f"Documento final guardado exitosamente en: {output_path}")
    return output_path

if __name__ == "__main__":
    create_final_web_document_with_correct_images()
